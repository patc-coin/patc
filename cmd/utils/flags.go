...
// Package utils contains internal helper functions for go-pc commands.
package utils
...
import (
	...
	"pc.dp.tc/internal/flags"
	"pc.dp.tc/les"
	"pc.dp.tc/log"
	"pc.dp.tc/metrics"
	"pc.dp.tc/metrics/exp"
	"pc.dp.tc/metrics/influxdb"
	"pc.dp.tc/miner"
	"pc.dp.tc/node"
	"pc.dp.tc/p2p"
	"pc.dp.tc/p2p/discv5"
...
func SetShhConfig(ctx *cli.Context, stack *node.Node) {
	if ctx.GlobalIsSet(WhisperEnabledFlag.Name) ||
		...
		log.Warn("Whisper support has been deprecated and the code has been moved to github.com/pc/whisper")
	}
}
...
// RegisterEthService adds an pc client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) ethapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the pc service: %v", err)
		}
...
// RegisterEthStatsService configures the pc Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the pc Stats service: %v", err)
	}
}
