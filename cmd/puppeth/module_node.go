...
// nodeDockerfile is the Dockerfile required to run an pc node.
var nodeDockerfile = `
FROM pc/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.pc/keystore/ && cp /signer.json /root/.pc/keystore/' >> geth.sh && \{{end}}
...
// an pc node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new pc node container to a remote machine via SSH,
