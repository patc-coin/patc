...
// newAlethGenesisSpec converts a go-pc genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-pc and aleth
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-pc/pull/11039,
...
// newParityChainSpec converts a go-pc genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-pc and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Genesis.Seal.pc.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.pc.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pypcGenesisSpec represents the genesis specification format used by the
// Python pc implementation.
type pypcGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPypcGenesisSpec converts a go-pc genesis block into a Parity specific
// chain specification format.
func newPypcGenesisSpec(network string, genesis *core.Genesis) (*pypcGenesisSpec, error) {
	// Only ethash is currently supported between go-pc and pypc
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pypcGenesisSpec{
