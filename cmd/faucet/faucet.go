// Copyright 2017 The go-pc Authors
// This file is part of go-pc.
//
// go-pc is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-pc is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-pc. If not, see <http://www.gnu.org/licenses/>.
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the pc protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for pc and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // pc address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an pc light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // pc protocol stack
	client *ethclient.Client   // Client connection to the pc chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the pc light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the pc service: %w", err)
	}
...
// close terminates the pc connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the pc address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://pc.dp.tc/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and pc address to fund on success.
func authTwitter(url string, token string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the pc address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No pc address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and pc address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the pc address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain pc address,
