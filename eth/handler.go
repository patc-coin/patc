...
import (
	...
	"pc.dp.tc/common"
	"pc.dp.tc/core"
	"pc.dp.tc/core/forkid"
	"pc.dp.tc/core/types"
	"pc.dp.tc/eth/downloader"
	"pc.dp.tc/eth/fetcher"
...
// support all the operations needed by the pc chain protocols.
...
// newHandler returns a handler for all pc chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("pc handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("pc peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerEthPeer(peer); err != nil {
		peer.Log().Error("pc peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if eth != nil {
		log.Debug("Removing pc peer", "peer", id)
