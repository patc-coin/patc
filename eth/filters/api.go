...
// information related to the pc protocol such als blocks, transactions and logs.
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(pc.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as pc.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria pc.FilterQuery
...
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(pc.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
