...
import (
	...
	"pc.dp.tc/accounts"
	"pc.dp.tc/accounts/usbwallet/trezor"
	"pc.dp.tc/common"
...
// or whether the pc app was not started on it.
...
// pc address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.pcAddress)
	if _, err := w.trezorExchange(&trezor.pcGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.pcTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.pcTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
