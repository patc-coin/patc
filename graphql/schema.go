...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte pc address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an pc account at a particular block.
    type Account {
    ...
    # Transaction is an pc transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an pc account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an pc account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an pc block by number or by hash. If neither is
