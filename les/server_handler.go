...
import (
	...
	"pc.dp.tc/core/types"
	"pc.dp.tc/ethdb"
	lps "pc.dp.tc/les/lespay/server"
	"pc.dp.tc/light"
	"pc.dp.tc/log"
	"pc.dp.tc/metrics"
	"pc.dp.tc/p2p"
	"pc.dp.tc/p2p/enode"
	"pc.dp.tc/p2p/nodestate"
	"pc.dp.tc/rlp"
	"pc.dp.tc/trie"
)
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light pc peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light pc response", "err", err)
			return err
		default:
		}
